...
func Initialize(additionalConfigWatchers ...*watcher.ConfigMapWatcher) *Accesses {
	...
	if err != nil {
		log.Infof("No valid prometheus config file at %s. Error: %s . Troubleshooting help available at: %s. Ignore if using cortex/thanos here.", promAddrs[scrapeKey], err.Error(), prom.PrometheusTroubleshootingURL)
	} else {
